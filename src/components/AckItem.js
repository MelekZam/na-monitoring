...
const AckItem = ({ item, users }) => {
      return (<View style={styles.ackItem}>
            <View style={styles.textBox}>
                ...
                <Text style={{color: color[parseInt(item.item.new_severity)],marginVertical:3}}><Text style={{color:'white'}}>New Severity : </Text>{severity[parseInt(item.item.new_severity)]}</Text></> : <Text style={{color:'white',marginVertical:3}}>Severity Unchanged</Text>}
                <Text style={{color:'white',marginVertical:3}}>Closed Problem : { item.item.action%2 ? 'Yes' : 'No'}</Text>
                <Text style={{color:'white',marginVertical:3}}>Unacknowledged : { item.item.action>=16 ? 'Yes' : 'No'}</Text>
            </View>
          </View>
      )
    } 
