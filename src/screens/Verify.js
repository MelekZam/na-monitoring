...
const Verify = ({navigation, route,dispatch}) => {
    ...
    const submitCode = async (code) => {
        const response = await axios.get(`http://192.168.43.215:3000/verify?phonenumber=216${route.params.phoneNumber}&code=${code}`) // send request to localhost node server to verify the code using twilio service
    ...
    const resendOTP = async () => {
        ...
        await axios.get(`http://192.168.43.215:3000/login?phonenumber=216${route.params.phoneNumber}&channel=sms`)
    }
    return (
        <View style={{flex: 1}}>
